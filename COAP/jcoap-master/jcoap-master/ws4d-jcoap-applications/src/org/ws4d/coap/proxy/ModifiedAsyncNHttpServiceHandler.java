...
public class ModifiedAsyncNHttpServiceHandler extends NHttpHandlerBase implements NHttpServiceHandler {

	private final HttpResponseFactory responseFactory;

	private NHttpRequestHandlerResolver handlerResolver;
	private HttpExpectationVerifier expectationVerifier;

	public ModifiedAsyncNHttpServiceHandler(final HttpProcessor httpProcessor,
			final HttpResponseFactory responseFactory, final ConnectionReuseStrategy connStrategy,
		...
		try {
			...
			if (request instanceof HttpEntityEnclosingRequest) {
				...
				if (consumingEntity == null) {
					consumingEntity = new ConsumingNHttpEntityTemplate(entityRequest.getEntity(),
	...
	private static void handleException(final HttpException ex, final HttpResponse response) {
	 ...
	 */
	private void processRequest(final NHttpServerConnection conn, final HttpRequest request)
		...
		try {
			...
			if (handler != null) {
				...
				HttpResponse response = this.responseFactory.newHttpResponse(ver, HttpStatus.SC_NOT_IMPLEMENTED,
						context);
				response.setParams(new DefaultedHttpParams(response.getParams(), this.params));
	...
	private void sendResponse(final NHttpServerConnection conn, final HttpRequest request, final HttpResponse response)
			throws IOException, HttpException {
		HttpContext context = conn.getContext();
		ServerConnState connState = (ServerConnState) context.getAttribute(CONN_STATE);

		// Now that a response is ready, we can cleanup the listener for the
		// request.
